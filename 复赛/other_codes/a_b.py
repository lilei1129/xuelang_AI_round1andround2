...
def A_B(file1,file2,percent1):
    ...
    for i in range(len(Y)):    
        if name1[i]==name2[i]:
            df.loc[number] = [name1[i],Y[i]*percent1+Y2[i]*(1-percent1)]
            number=number+1
        else:
            print('error')
#    for i in range(len(Y)):
#        for j,name in enumerate(name2):
#            if name1[i]==name2[j]:
#                   df.loc[number] = [name1[i],Y[i]*percent1+Y2[j]*(1-percent1)]
#                   number=number+1
#                break
#                
#    
...
file1="submit/submit_20180831_212732_11.csv"
file2="submit/submit_20180831_183615.csv"
# 0.5是指第一个csv文件的权重
A_B(file1,file2,0.4)
