...
model11= load_model("./Xception_models_3/cut512-19-0.9858.h5")
##
...
for line in open("submit_20180824_143001.csv"):
    if j==-1:#删除文件头
        ...
        qq=float('%.6f' % qq) 
        if Y>=0.5:# 二分类预测认为是瑕疵
            imgPath='../data/xuelang_round2_test_a_20180809/'+file
            pArrayA = predictCutPic11(imgPath,512,512,256,model11,padding=False,paddingSize=256)           
            ...
            if np.max(test)<1:#没有明显瑕疵，此时的输出为前三个norm预测的平均
                ...
                for i in range(7):
                    for j in range(9):
                        if test[i][j]!=0.0:
                            ...
                            if(pArrayA[i*7+j,int(test[i][j])] >0.65):
                                percent_list[int(test[i][j])]=percent_list[int(test[i][j])]+pArrayA[i*7+j,int(test[i][j])]                    
                                #各种瑕疵在小框里面出现的次数
                ...
                mua=np.sort(count_list)[-1] 
                mua_0=np.sort(count_list)[-2]         
                defect_class_num=0
                #这张图片里面被预测出来的瑕疵种类     
                ...
                if defect_class_num==0:
                    ...
                    if mua>mua_0:
                        ...
                        if mua>3:#如果出现次数大于3次
                            ...
                            for num in range(11):
                                df.loc[number] = [imgPath.split('/')[-1]+'|'+LABELS[num],deal_pList_ff(np.mean(sorted_big_boss[-3:,num]))]
                                number=number+1
                        elif mua==2:
                            #取出现概率最大的2次平均得到最终的概率值
                            for num in range(11):
                                df.loc[number] = [imgPath.split('/')[-1]+'|'+LABELS[num],deal_pList_ff(np.mean(sorted_big_boss[-2:,num]))]
                                number=number+1
                        else:
                            #取出现概率最大的1次得到最终的概率值
                            pArrayB=pArrayA[:,1:]
                        ...
                        #如果出现次数最多的有两种瑕疵，且其出现次数一致，这个时候按照平均概率最大的处理
            ...
            df.loc[number] = [file+'|'+LABELS[0],qq]
            number=number+1    
            ssa=float(Y*0.1)
            m=float('%.6f' % ssa) 
            if m >=0.999998:
                ...
                m = 0.000001
            for i in range(10):
                df.loc[number] = [file+'|'+LABELS[i+1],m]  
                number=number+1  
                print(number)           
