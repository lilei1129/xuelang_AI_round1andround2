...
modelA = load_model("./Xception_models_3/cut320-44-0.7665.h5")
test=np.zeros(shape=(15,11))
number=0
pAa=[]
pArrayListA = []
pListB = []
...
for imgPath in tqdm(imgPathList):
    pArrayA = predictCutPic11(imgPath,320,320,160,modelA,padding=False,paddingSize=160)
    for i,pAa in enumerate(pArrayA):
        if(pAa[0]<0.5):                        
            ...
            p_xc=0
        test[i//11][i%11]=p_xc    
    df2 = pd.DataFrame(test,index=range(15),columns=range(11))
#    fig = plt.figure()
...
#    the_table.scale(2.5,2.58)    
    ...
    if np.max(test)<1:#没有明显瑕疵，此时的输出为所有预测值的平均
        for num in range(11):
            df.loc[number] = [imgPath.split('/')[-1]+'|'+LABELS[num],np.mean(pArrayA[:,num])]
            number=number+1
    else :#存在瑕疵,判断何种瑕疵出现的次数最多
        ...
        max_defect=np.argmax(count_list[0:])
        if mua>1:
            boss=np.zeros(shape=(1,11))
        else:
            boss=np.zeros(shape=(1,11))
            
        big_boss=np.zeros(shape=(int(mua),11))
        n=0
        for i in range(15):
            for j in range(11):
                if test[i][j]==max_defect+0.0:            
                    big_boss[n]=pArrayA[i*11+j]
                    n=n+1
        boss=big_boss[0]
        for i in range(n):
            if big_boss[i,int(max_defect)]>boss[int(max_defect)]:
                boss=big_boss[i]
        for num in range(11):
            df.loc[number] = [imgPath.split('/')[-1]+'|'+LABELS[num],boss[num]]
            number=number+1
#    if number>220:
#        break
...
#    m,n,a,s=getPosition(imgPath,pArrayA)
#    df.loc[number] = [imgPath+'|'+s,a]
#    number=number+1
#df.to_csv(("./submit/submit_"+datetime.datetime.now().strftime('%Y%m%d_%H%M%S') + ".csv"), index=False,sep=',')
#        
#    Pnorm=0
#    Pdefect=0
#    for pAa in pArrayA:
#        Pdefect=Pdefect+pAa[0]
