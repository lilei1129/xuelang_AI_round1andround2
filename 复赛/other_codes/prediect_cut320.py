...
modelA = load_model("./Xception_models_3/cut320-56-0.8072.h5")
...
for imgPath in tqdm(imgPathList):
    ...
    for i,pAa in enumerate(pArrayA):
        if(pAa[0]<0.5):                        
            ...
            p_xc=0
        test[i//11][i%11]=p_xc
    feifei_num=feifei_num+1
    if(feifei_num<20):
        df2 = pd.DataFrame(test,index=range(15),columns=range(11))
        for i in range(15):
            df2.loc[i] = [test[i,0],test[i,1],test[i,2],test[i,3],test[i,4],test[i,5],test[i,6],test[i,7],test[i,8],test[i,9],test[i,10]]
            #利用test来判断有无瑕疵
        df2.to_csv(("../data/"+ (imgPath.split('/')[-1]).split('.')[0]+".csv"), index=False,sep=',')                    
    #利用test来判断有无瑕疵
        ...
        for i in range(15):
            for j in range(11):
                if test[i][j]!=0.0:
                    ...
                    if(pArrayA[i*11+j,int(test[i][j])] >0.65):
                        percent_list[int(test[i][j])]=percent_list[int(test[i][j])]+pArrayA[i*11+j,int(test[i][j])]                    
                        #各种瑕疵在小框里面出现的次数
