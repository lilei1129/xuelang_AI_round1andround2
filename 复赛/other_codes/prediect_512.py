...
#modelA = load_model("./model/Xception_models_1/model.hdf5", custom_objects={'new_loss': new_loss})
...
for imgPath in tqdm(imgPathList):
    pArrayA = predictCutPic11(imgPath,512,512,256,model11,padding=False,paddingSize=160)
        ...
        for i in range(7):
            for j in range(9):
                if test[i][j]!=0.0:
                    ...
                    if(pArrayA[i*9+j,int(test[i][j])] >0.65):
                        percent_list[int(test[i][j])]=percent_list[int(test[i][j])]+pArrayA[i*9+j,int(test[i][j])]                    
                        #各种瑕疵在小框里面出现的次数
